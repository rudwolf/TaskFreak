...
class TznDbConnection
{
    ...
    function __construct($host = null, $user = null, $pass = null, $base = null)
    {
        if ($host && $user && $base) {
            $this->_dbHost = $host;
            $this->_dbUser = $user;
            ...
            $this->_dbHost = defined('TZN_DB_HOST') ? TZN_DB_HOST : '';
            $this->_dbUser = defined('TZN_DB_USER') ? TZN_DB_USER : '';
            $this->_dbPass = defined('TZN_DB_PASS') ? TZN_DB_PASS : '';
            $this->_dbBase = defined('TZN_DB_BASE') ? TZN_DB_BASE : '';

            // Debug logging
            error_log("DB Config - Host: " . $this->_dbHost . ", User: " . $this->_dbUser . ", Base: " . $this->_dbBase);
        }
        $this->_critical = true;
    }
  ...
  $this->_setObjectProperties("setSql",$objData);
    ...
    */
    ...
    function pSort(
        ...
        $field,
        $style = '',
        $imgAsc = TZN_DB_ASC_OFF,
        $imgAscActive = TZN_DB_ASC_ON,
        $imgDesc = TZN_DB_DESC_OFF,
        $imgDescActive = TZN_DB_DESC_ON
    ) {
        $begin = '<a href="' . $link;
        if (preg_match('/\?/', $link)) {
            $begin .= '&';
    ...
    function pPrevious(
        ...
        $param,
        $text = "&lt;",
        $styleOn = TZN_DB_PAGING_ENABLED,
        $styleOff = TZN_DB_PAGING_DISABLED
    ) {
        // link = page filename + original parameters
        // param = name of variable parameter for paging
        // current = current page (integer)
