...
if (!defined("TZN_DATE_FRM")) {
    if (defined('TZN_DATE_US_FORMAT') && constant('TZN_DATE_US_FORMAT')) {
...
class Tzn
{
    ...
    function getBol($keyval, $default = null)
    {
        if (is_object($this)) {
            ...
            } elseif (isset($this->_properties[$keyval]['value'])) {
                // Check in _properties array
                $value = $this->_properties[$keyval]['value'];
            } else {
                $value = $default;
            }
        } else {
    ...
    static function _getUserTZ($tz = TZN_TZDEFAULT)
    {
        if (is_numeric($tz)) {
            $tz = intval($tz);
        } else if ($tz == 'server') {
            ...
            $userTz = $_SESSION['tznUserTimeZone'] ?? null;
            $tz = intval($userTz);
    ...
    static function redirect($url, $message = '', $forceRef = false)
    {
        if (@constant('TZN_TRANS_ID')) {
    ...
    static function concatUrl($url, $param)
    {
        // hash
        $hash = '';
        if ($pos = strpos($url, '#')) {
